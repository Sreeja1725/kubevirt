...
import (
	...
	nodelocalhotplug "kubevirt.io/kubevirt/pkg/virt-handler/node-local-hotplug/server"
...
func (app *virtHandlerApp) Run() {
	...
	if app.clusterConfig.NodeLocalHotplugEnabled() {
		nlhCtx, nlhCancel := context.WithCancel(context.Background())
		go func() { <-stop; nlhCancel() }()
		nlhServer := nodelocalhotplug.NewServer(app.HostOverride, app.virtCli, app.clusterConfig)
